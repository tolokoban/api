...
import Tokenizer, { EnumToken, IToken } from "./tokenizer"
import {
    IType,
    ITypeObject,
    ITypeAlias,
...
import { ensureFloat, ensureInt, isObject } from "../tools/type-guards"
...
function convertEnumToErrors(obj: ITypeEnum): IError[] {
    return Object.keys(obj.enums).map(name => ({
        name,
        value: obj.enums[name].value,
        comment: obj.enums[name].comment,
    }))
}
